...
cmd = "open -a Safari test_v2.html"
...
def main():
    ...
    features = features_v2.features(time)

    for i in range(len(chapters)):
        with open(JS_FILE, "a", encoding="utf-8") as f:
                f.write("$(\'.chaps\').append(\'<tr id=\"chap"+ chapters[i][1] + "\" class=\"chap\" onclick=\"clicked(" + chapters[i][1] + ")\"><td>" + chapters[i][0] + chapters[i][1] + ":"+ features[i] + "<span class=\"text-muted fs-5\">" + chapters[i][2] + "</span></td></tr>\');\n")
...
runcmd = subprocess.Popen(cmd.split())